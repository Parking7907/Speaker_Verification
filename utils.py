...
def get_cossim(embeddings, centroids):
    ...
    utterance_centroids = get_utterance_centroids(embeddings)#4by 5by 256, 자기 자신 뺀 친구들 4개의 평균
    ...
    cos_same = F.cosine_similarity(embeddings_flat, utterance_centroids_flat) #모양 : 20  (결과)
    ...
    centroids_expand = centroids.repeat((num_utterances * embeddings.shape[0], 1))#80*256 Enrollment 친구들이랑 이제 비교해보자!
    embeddings_expand = embeddings_flat.unsqueeze(1).repeat(1, embeddings.shape[0], 1) # 20 * 4 * 256
    embeddings_expand = embeddings_expand.view(
        embeddings_expand.shape[0] * embeddings_expand.shape[1],
        embeddings_expand.shape[-1]
    )#80 by 256으로 얘는 비교군
    cos_diff = F.cosine_similarity(embeddings_expand, centroids_expand)#80(결과) 80 by 256 두개 박아서 차이값
